...
<head>
    ...
    <title>Iniciar sesión | The Flying Bookstore</title>
    ...
    <div class="w-full min-h-screen flex items-center justify-center">
        <div class="w-full max-w-5xl rounded-2xl shadow-none flex flex-col md:flex-row overflow-hidden border border-gray-200" style="min-height: 600px;">
            ...
            <div class="flex-1 flex flex-col justify-center px-10 py-12 bg-[#FCFCF6] relative">
                ...
                <h2 class="text-3xl font-bold mb-8 text-gray-800 mt-20 md:mt-0">Bienvenido</h2>
                @if(session('status'))
        ...
        @csrf
                    <div>
                        ...
                        <div class="relative">
                            <input id="email" name="email" type="email" required autofocus autocomplete="username" placeholder="correo@email.com" value="{{ old('email') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
                        ...
                        <div class="relative">
                            <input id="password" name="password" type="password" required autocomplete="current-password" placeholder="••••••••" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <button type="submit" class="w-full bg-[#0A2342] text-white py-2 rounded-lg font-semibold hover:bg-[#16335B] transition">Iniciar sesión</button>
                </form>
                <div class="mt-6 text-center text-sm text-gray-700">
                    ¿No tienes una cuenta? <a href="{{ route('register') }}" class="text-[#F76C6C] font-semibold hover:underline">Aquí</a>
                </div>
        </div>
            <!-- Columna derecha: imagen -->
            <div class="hidden md:flex flex-1 bg-[#FCFCF6] items-center justify-center p-6">
                <img src="{{ asset('frame1.png') }}" alt="Ilustración" class="w-full h-full object-contain rounded-2xl" style="max-height: 540px;">
        </div>
        </div>
        </div>
    <script>
