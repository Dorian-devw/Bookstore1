...
<div class="container mx-auto py-10 max-w-2xl">
    ...
    <form action="{{ route('admin.libros.update', $libro->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded shadow p-6 space-y-4">
        @csrf
        <div>
            <label class="block font-semibold mb-1">Título</label>
            <input type="text" name="titulo" value="{{ old('titulo', $libro->titulo) }}" class="w-full border rounded p-2" required>
            @error('titulo')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block font-semibold mb-1">Descripción</label>
            <textarea name="descripcion" class="w-full border rounded p-2">{{ old('descripcion', $libro->descripcion) }}</textarea>
        </div>
        <div class="flex gap-4">
            <div class="w-1/2">
                <label class="block font-semibold mb-1">Precio</label>
                <input type="number" step="0.01" name="precio" value="{{ old('precio', $libro->precio) }}" class="w-full border rounded p-2" required>
                @error('precio')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>
            <div class="w-1/2">
                <label class="block font-semibold mb-1">Stock</label>
                <input type="number" name="stock" value="{{ old('stock', $libro->stock) }}" class="w-full border rounded p-2" required>
            ...
            <div class="w-1/2">
                ...
                <select name="categoria_id" class="w-full border rounded p-2" required>
                    ...
                    @foreach($categorias as $cat)
                        <option value="{{ $cat->id }}" @if(old('categoria_id', $libro->categoria_id) == $cat->id) selected @endif>{{ $cat->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="w-1/2">
                ...
                <select name="autor_id" class="w-full border rounded p-2" required>
                    ...
                    @foreach($autores as $autor)
                        <option value="{{ $autor->id }}" @if(old('autor_id', $libro->autor_id) == $autor->id) selected @endif>{{ $autor->nombre }}</option>
                    @endforeach
                </select>
        ...
        <div class="flex gap-4">
            <div class="w-1/2">
                ...
                <input type="text" name="idioma" value="{{ old('idioma', $libro->idioma) }}" class="w-full border rounded p-2" required>
                @error('idioma')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>
            <div class="w-1/2">
                <label class="block font-semibold mb-1">Valoración</label>
                <input type="number" step="0.1" min="0" max="5" name="valoracion" value="{{ old('valoracion', $libro->valoracion) }}" class="w-full border rounded p-2">
        ...
        <div>
            ...
            <input type="date" name="publicado_en" value="{{ old('publicado_en', $libro->publicado_en) }}" class="w-full border rounded p-2">
        </div>
        <div class="text-right">
            <button type="submit" class="bg-blue-600 text-white px-8 py-2 rounded hover:bg-blue-700">Actualizar libro</button>
